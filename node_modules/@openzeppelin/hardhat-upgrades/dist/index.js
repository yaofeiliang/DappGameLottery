...
config_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY, async (args, hre, runSuper) => {
    const { readValidations, ValidationsCacheOutdated, ValidationsCacheNotFound } = await Promise.resolve().then(() => __importStar(require('./validations')));
    try {
...
config_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_COMPILE, async (args, hre, runSuper) => {
    ...
    const { writeValidations } = await Promise.resolve().then(() => __importStar(require('./validations')));
    // TODO: patch input
